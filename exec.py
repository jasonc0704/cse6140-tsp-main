...
class LS():
    ...
    def cost_func(self,sol):
        ...
        for i in range(1,len(sol)):
            Sum  += round(np.linalg.norm(self.pos[sol[i],:] - self.pos[sol[i-1],:]))
        Sum += round(np.linalg.norm(self.pos[sol[-1],:] - self.pos[sol[0],:]))
        return Sum
...
def main(tsp,algo,cutoff,seed):
    ...
    if algo == 'Approx':
        ...
        with open(os.path.join(output_dir, sol_file), 'w') as f:
            f.write(str(int(distance)) + "\n")
            f.write(','.join(str(item) for item in best_tour))
        f.close()
